@echo off
REM Fix All Issues with Correct Database Name Script
REM Plant Disease Classification System

echo ========================================
echo Fix All Issues (Correct Database)
echo ========================================
echo.
echo This script will fix all issues using
echo the correct database name: plant_disease_db
echo.
...
REM Step 1: Update database names in all files
echo [STEP 1] Updating database names in all files...
if exist "update-database-names.bat" (
    echo [INFO] Running database names update...
    call update-database-names.bat
    if %errorlevel% neq 0 (
        echo [WARNING] Database names update had issues, continuing...
    )
) else (
    echo [INFO] Database names update script not found, skipping...
)
echo.

REM Step 2: Update environment variables
echo [STEP 2] Updating environment variables...
if exist "update-env-database.bat" (
    echo [INFO] Running environment update...
    call update-env-database.bat
    if %errorlevel% neq 0 (
        echo [WARNING] Environment update had issues, continuing...
    )
) else (
    echo [INFO] Environment update script not found, skipping...
)
echo.

REM Step 3: Check PostgreSQL connection
...
if %errorlevel% neq 0 (
    ...
    echo   2. Verify database 'plant_disease_db' exists
    echo   3. Check username and password
    echo   4. Check port 5432 is open
    echo.
    pause
    exit /b 1
)
...
REM Step 5: Fix database issues
echo [STEP 5] Fixing database issues...
...
REM Step 6: Setup login system
echo [STEP 6] Setting up login system...
...
REM Step 7: Fix predictions table
echo [STEP 7] Fixing predictions table...
...
REM Step 8: Test login endpoint
...
REM Step 9: Test validation and limits
echo [STEP 9] Testing validation and limits...
...
echo   - Database names updated to plant_disease_db
