...
CREATE INDEX IF NOT EXISTS idx_plants_severity ON plants(severity);
...
AND tablename IN ('users', 'plants', 'predictions', 'detection_history', 'guest_detection_limits')
...
AND event_object_table IN ('users', 'plants', 'predictions', 'detection_history', 'guest_detection_limits')
...
SELECT 'Sample Detection History' as section, 
...
SELECT 'Guest Detection Limits Status' as section,
    ip_address,
    detection_count,
    CASE 
...
-- Total detections per user type
SELECT 
    COALESCE(u.username, 'Guest') as user_type,
    COUNT(*) as total_detections,
    AVG(dh.confidence) as avg_confidence
FROM detection_history dh
LEFT JOIN users u ON dh.user_id = u.id
GROUP BY u.username
...
GROUP BY plant_class
ORDER BY detection_count DESC;

-- Guest detection limits overview
SELECT 
    COUNT(*) as total_ips,
...
SELECT 'You can now start using the Plant Disease Classification System' as next_step;
